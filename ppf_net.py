...
tf.app.flags.DEFINE_integer('NUM_STEPS_PER_DECAY', 250000,
...
def loss(logpros, labels):
    cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(logpros, tf.to_float(labels), name='xentropy')
...
def inference(images):
    ...
    with tf.variable_scope('scores') as scope:
        ...
        scores = tf.nn.conv2d(avg_pool, weights, [1,1,1,1], padding='VALID')
